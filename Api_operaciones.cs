...
public class ApiOperaciones {
    ...
    public ApiOperaciones(ApiOperacionesOptions opciones) {
    ...
    public async Task<ApiResponse> ApiAutorizaAsync(string keyLogin, string idSesion, string origenUri = "/") {     

        var rta = new ApiResponse { 
            status = 400,
            inStatus = 400,
        ...
        try {
            ...
            Console.WriteLine($"catchhh: {ex} ");
            return rta;
        }
        return rta;
    }

    public async Task<ApiResponse> api_getToken(string codigoAutorizacion) {     
        var rta = new ApiResponse { 
            status = 400,
            inStatus = 400,
        ...
        try {
            using (HttpClient httpClient = new HttpClient()) {
                ...
                var result = await response.Content.ReadAsStringAsync();
                //var data = JsonSerializer.Deserialize<ApiResponseOther>(result);
                var data = Deserialize(result);
    ...
    public async Task<ApiResponse> api_getPerfil(string token) {     
            var rta = new ApiResponse { 
                status = 400,
                inStatus = 400,
     ...
     public async Task<ApiResponse> api_verificaSesionToken(string token) {     
            var rta = new ApiResponse { 
                status = 400,
                inStatus = 400,
                data = new Dictionary<dynamic, dynamic?> {},
            ...
            try {
                ...
                if (token != "" && token != null)
                {
                    using (HttpClient httpClient = new HttpClient()) {
                        ...
                        var result = await response.Content.ReadAsStringAsync();
                        Console.WriteLine($"string result Token: {result} ");
                        var data = Deserialize(result);
                        if (response.StatusCode == System.Net.HttpStatusCode.OK) {
                            rta.status = 200;
                            rta.inStatus = 200;
      ...
      public async Task<ApiResponse> api_cerrarSesion(string token) {     
            var rta = new ApiResponse { 
                status = 400,
                inStatus = 400,
                data = new Dictionary<dynamic, dynamic?> {},
    ...
    public async Task<ApiResponse> api_getSolicitud(string token, string solicitud) {     
            var rta = new ApiResponse { 
                status = 400,
                inStatus = 400,
                data = new Dictionary<dynamic, dynamic?> {},
    ...
    public async Task<ApiResponse> api_validaOTP(string token, string origenUri) {     
            var rta = new ApiResponse { 
                status = 400,
                inStatus = 400,
                data = new Dictionary<dynamic, dynamic?> {{"Redirect", null}},
    ...
    public async Task<ApiResponse> api_validaFaceTec(string token, string origenUri) {     
            var rta = new ApiResponse { 
                status = 400,
                inStatus = 400,
                data = new Dictionary<dynamic, dynamic?> {{"Redirect", null}},
...
public class ApiOperacionesOptions {
...
public class opcionesMiddleware
{
