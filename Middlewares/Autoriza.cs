...
public class Autoriza
{
    ...
    public Autoriza(RequestDelegate next, string rutaLogin, ApiOperaciones api)
    {
        _next = next;
        _ruta = rutaLogin;
        _api = api;
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        if ("MiddlewareVerificaSesionToken" == _ruta || context.Request.Path == _ruta)
        {
            try
            {
                ...
                ApiResponse rta = await _api.ApiAutorizaAsync(context.Request.Cookies["cdm-keyLogin"] ?? "", session!, (string)context.Items["originUrl"]! ?? "/");
